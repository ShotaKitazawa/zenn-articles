...
マルチプロバイダ向けの CD ツールですが、今回は [Quick Start](https://pipecd.dev/docs/quickstart/) に沿った Kubernetes へのデプロイを検証しました。
...
まず WebUI へアクセスするためにポートフォワーディングを実行します。

```bash
kubectl -n pipecd port-forward svc/pipecd 8080
```

その後、WebUI で以下を設定します。それぞれ以下のような役割があります。
...
Piped も ControlPlane と同様に以下の Kubernetes マニフェストフォーマットの設定ファイルが実行時引数で渡されます。 `.spec.pipedID` には 1 つ前の手順で発行した Piped ID を入力します。
...
また個人的に大きな利点として、開発されている方が日本人ということも挙げられます。自分は英語が得意ではないので、英語で書かれた GitHub や Slack のやりとりは頑張って読むのですが、いざというときに Twitter 等にて日本語で開発者の方とやり取りできるのはとてもありがたいです。
