...
会社の Advent Calendar での参加ですが仕事でやっていることを話さなければいけないというレギュレーションは特に無かったため、[Qiita: Kubernetes Advent Calendar 2020](https://qiita.com/advent-calendar/2020/kubernetes) の 11 日目に出した [GitOps を実現する CD ツール、PipeCD が良さそうという話](https://zenn.dev/kanatakita/articles/c22b489c0be273144215) の記事関連で趣味開発の話をしようと思います。

今回の話は実装してみた系 & 実装してみてつまずいたところの備忘録的な記事になります。

# 導入

先日、 PipeCD という CD ツールについて [GitOps を実現する CD ツール、PipeCD が良さそうという話](https://zenn.dev/kanatakita/articles/c22b489c0be273144215) という記事を書きました。

PipeCD は現状最初の stable release を目指すフェーズにおり比較的新しいソフトウェアです。そのため自身の勉強と PipeCD の界隈を少しでも盛り上げる意味を兼ねて、PipeCD を Kubernetes API から操作できるような CRD + Controller を Kubebuilder を用いて実装してみました。

(前の記事で K8s と密結合でないことを PipeCD の利点に上げときながらなんで Operator で Kubernetes API を噛ませるようにしたんやねんという感じですが自学用途ということで、、)

ソースコードは以下になります。
**あくまで自学用途なので動作は保証できませんし、権限も大きめに付けているのでご注意下さい！**
...
* `ControlPlane.spec.config` 以下の宣言を `Server` コンポーネント起動時の引数に渡す設定ファイルに整形し ConfigMap リソースとして保存
...
* `Piped.spec.config` 以下の宣言を `Piped` コンポーネント起動時の引数に渡す設定ファイルに整形し ConfigMap リソースとして保存
* Piped 用の各種リソースをデプロイ
    * 上記で払い出した Piped Secret をここで生成する Pod に渡すことで、Piped Pod が 該当 Project, 該当 Environment に紐づく Piped として登録される
...
順番であることの必要性については [備忘録:オブジェクトの削除処理の順番を制御したい](TODO) にて後述します。
...
gRPC WebService インタフェースでは認可に JWT (JSON Web Token) を利用しており、署名アルゴリズムに HS256 を用いています。そのため HS256 の 共有シークレットを `Server` コンポーネントと `Piped` コンポーネント間で共有することで強引に認証認可を突破する方法を取りました。

Piped (gRPC クライアント) にて上記のような JWT を生成している部分のソースコードは[このあたり](https://github.com/ShotaKitazawa/pipecd-operator/blob/fcef64f94111b0370f2822c70d0e3c981e62481d/infrastructure/pipecdweb/pipecd_web_client_generator.go#L36-L52) です。
...
pipecd-operator では control-plane-config.yaml, piped-config.yaml (以下 実設定ファイル) の内容をそれぞれ `ControlPlane.spec.config`, `Piped.spec.config` に記述できるようにしました。
...
上記で述べたとおりまだ実装途中な部分 (例. PipeCD Application を操作する Controller) や PipeCD の最新版に追従しきれていない部分 (例. API Key を用いた PipeCD との通信) 、この Advent Calendar の締切駆動開発をしてしまったせいで不十分な部分 (パッケージ構成が雑なことやテストが無いこと等) が色々あるので、これからも継続的に実装を進められたらなと思います。
